...
tf = 'test_currencychangeinproject'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100220
class Currencychangeinproject(unittest.TestCase):
    def test_currencychangeinproject(self):
        try:
            i = 10
            while i<11:
                ...
                time.sleep(1)
                browser = Updatedetails.projectdetailscurrency(browser,projectname,projectreference,projectdescription,projecttype,projectvalue) #Pass the values from excel
