...
#filename = 'TestCase-100305-{0}.png'.format(ptime)
tf = 'test_plantemplatestartasforecastdate'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100305
class Plantemplatestartasforecastdate(unittest.TestCase):
    def test_plantemplatestartasforecastdate(self):
        try:
            ...
            NewTender = tenderclass.TenderCreationforstartdate(browser)
            time.sleep(3)
            ...
            logs.info("Test Case No : 100305 Passed Successfully")
            ...
            logs.error("Validation with Test Case No: 100305 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100305 failed")
            browser.implicitly_wait(5)
        finally:
