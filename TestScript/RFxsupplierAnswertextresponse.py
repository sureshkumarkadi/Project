...
tf = 'test_RFxsupplieranswertextresponse'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = RFx-5
class RFxsupplieranswertextresponse(unittest.TestCase):
    def test_RFxsupplieranswertextresponse(self):
        try:
            ...
            browser = RFxAnswers.SubmitRFxanswers(browser)
            time.sleep(1)

            RFxAnswerstab1 = DataDriver()
            time.sleep(1)
            RFxsuccessmessage_path = RFxAnswerstab1.readfromXML(folder_path+'\Object\RFx.xml','eTender','successmessageforsupplier')
            RFxsuccessmessage = browser.find_element_by_xpath(RFxsuccessmessage_path)
            time.sleep(1)

            self.assertEqual(RFxsuccessmessage.text,'Responses submitted successfully.')
