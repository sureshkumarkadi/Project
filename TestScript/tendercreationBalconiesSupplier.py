...
#filename = 'TestCase-100363-{0}.png'.format(ptime)
tf = 'test_TendercreationBalconiesSupplier'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100363
class TendercreationBalconiesSupplier(unittest.TestCase):
    def test_TendercreationBalconiesSupplier(self):
        try:
            i = 2
            while i<3:
                ...
                self.assertEqual(tenderstatus[3].text,'Active')
                #self.assertEqual(tendertype,'SUPPLIER')
                #self.assertEqual(tenderstatus[14].text,todaysdate)
                ...
                #self.assertEqual(tendername[5].text,'Balconies1')

                time.sleep(3)
                i = i + 1
        except Exception:
                logs.error("Validation with Test Case No: 100363 failed")
                browser.save_screenshot(fullpath)
                traceback.print_exc(file=sys.stdout)
                self.fail("Test Case No: 100363")
                browser.implicitly_wait(5)
        finally:
