...
#filename = 'TestCase-100369-{0}.png'.format(ptime)
tf = 'test_TendercreationBalustradingSupplier'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100369
class TendercreationBalustradingSupplier(unittest.TestCase):
    def test_TendercreationBalustradingSupplier(self):
        try:
            i = 8
            while i<9:
                ...
                self.assertEqual(tenderstatus[6].text,'Active')
                #self.assertEqual(tendertype,'SUPPLIER')
                #self.assertEqual(tenderstatus[26].text,todaysdate)
                ...
                #self.assertEqual(tendername[8].text,'Balustrading2')

                time.sleep(3)
                i = i + 1
        except Exception:
                logs.error("Validation with Test Case No: 100369 failed")
                browser.save_screenshot(fullpath)
                traceback.print_exc(file=sys.stdout)
                self.fail("Test Case No: 100369")
                browser.implicitly_wait(5)
        finally:
