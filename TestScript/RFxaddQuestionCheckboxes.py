...
tf = 'test_RFxaddQuestioncheckboxes'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = RFX-3
class RFxaddQuestioncheckboxes(unittest.TestCase):
    def test_RFxaddQuestioncheckboxes(self):
        try:
            ...
            browser = RFxQuestions.RFxCheckbox(browser)
            time.sleep(2)

            browser = RFxQuestions.selectdropdown(browser)
            time.sleep(2)

            browser = RFxQuestions.selectCheckbox(browser)
            time.sleep(2)
            ...
            time.sleep(1)
            templateoption = browser.find_elements_by_xpath(templateoptions.readfromXML(folder_path+'\Object\RFx.xml','eTender','templateoptions'))
            time.sleep(1)

            self.assertEqual(templateoption[2].text,'Yes')
            self.assertEqual(templateoption[3].text,'No')
