...
tf = 'test_RFxsupplieranswerCheckboxes'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = RFx-7
class RFxsupplieranswerCheckboxes(unittest.TestCase):
    def test_RFxsupplieranswerCheckboxes(self):
        try:
...
##            self.assertEqual(RFxmultipleoptions[0].text,'No')
##            self.assertEqual(RFxmultipleoptions[1].text,'Yes')

##            self.assertEqual(RFxmultipleoptions[2].text,'Yes')
##            self.assertEqual(RFxmultipleoptions[3].text,'No')
