...
#filename = 'TestCase-100240-{0}.png'.format(ptime)
tf = 'test_currencychangeinprojectEURO'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100240
class CurrencychangeinprojectEURO(unittest.TestCase):
    def test_currencychangeinprojectEURO(self):
        try:
            i = 2
            while i<3:
                ...
                value = numbers.format_currency(200,'EUR',locale='en')
                ...
                i = i + 1
                logs.info("Test Case No : 100240 Passed Successfully")
                ...
                logs.error("Validation with Test Case No: 100240 failed")
                browser.save_screenshot(fullpath)
                traceback.print_exc(file=sys.stdout)
                self.fail("Test Case No: 100240 failed")
                browser.implicitly_wait(5)
        finally:
