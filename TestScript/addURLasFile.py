...
#Test case Number = TC1026
class AddURLasFile(unittest.TestCase):
    def test_addURLasFile(self):
        try:
            ...
            browser = tenderDetails.documentasfile(browser)
            time.sleep(1)

            Documentname1 = DataDriver()
            time.sleep(1)
            Documentname_path = Documentname1.readfromXML(folder_path+'\Object\Project.xml','eTender','documentname')
            Documentname = browser.find_element_by_xpath(Documentname_path)
            time.sleep(1)
            self.assertEqual(Documentname.text,'URLasfile')
