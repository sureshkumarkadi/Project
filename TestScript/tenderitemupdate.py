...
#filename = 'TestCase-100312-{0}.png'.format(ptime)
tf = 'test_Tenderitemupdate'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100312
class Tenderitemupdate(unittest.TestCase):
    def test_Tenderitemupdate(self):
        try:
            ...
            itemquantity = []
            time.sleep(1)
            itemquantity_path = itemsintender1.readfromXML(folder_path+'\Object\Itemsobject.xml','eTender','itemquantity')
            itemquantity = browser.find_elements_by_xpath(itemquantity_path)
            self.assertEqual(itemquantity[0].text,'20')

            itemrate = []
            time.sleep(1)
            itemrate_path = itemsintender1.readfromXML(folder_path+'\Object\Itemsobject.xml','eTender','itemrate')
