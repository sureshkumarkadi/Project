...
#filename = 'TestCase-100282-{0}.png'.format(ptime)
tf = 'test_Plantemplatenotesupdate'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100282
class Plantemplatenotesupdate(unittest.TestCase):
    def test_Plantemplatenotesupdate(self):
        try:
            ...
            time.sleep(5)
            browser = tendertemplate.notesfortenderplan(browser) #Click on Notes
            time.sleep(1)
            browser = tenderDetails.updatingNotesfortenderplan(browser) # adding notes
            time.sleep(1)
            browser = tendertemplate.notesfortenderplan(browser) #Click on Notes
            time.sleep(1)
            ...
            #self.assertEqual(actualnotes,'Update ')

            logs.info("Test Case No : 100282 Passed Successfully")
            time.sleep(1)

            browser = tendertemplate.notesfortenderplansave(browser)
            time.sleep(1)
            browser = tendertemplate.tenderplansave(browser)
            time.sleep(1)
        except Exception:
            logs.error("Validation with Test Case No: 100282 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100282 failed")
            browser.implicitly_wait(5)
        finally:
