...
#filename = 'TestCase-TC1050-{0}.png'.format(ptime)
tf = 'test_hintinSuperadminonehint'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case no:TC1050
class HintinSuperadminonehint(unittest.TestCase):
    def test_hintinSuperadminonehint(self):
        try:
            ...
            browser = Hintdisplay.stagesmenu(browser)
            time.sleep(1)
            ...
            self.assertEqual(hinttitle.text,'Stage')
            ...
            self.assertEqual(hintcontent.text,'Stage Creation can be done here')
            ...
            logs.error("Validation with Test Case No: TC1050 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: TC1050 failed")
            browser.implicitly_wait(5)
        finally:
            time.sleep(1)
