...
#filename = 'TestCase-100283-{0}.png'.format(ptime)
tf = 'test_Plantemplatenotesdelete'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100283
class Plantemplatenotesdelete(unittest.TestCase):
    def test_Plantemplatenotesdelete(self):
        try:
            ...
            time.sleep(1)
            browser = tendertemplate.deletenotesfortenderplan(browser) #clear/delete Notes
            time.sleep(3)
            ...
            logs.info("Test Case No : 100283 Passed Successfully")
            time.sleep(1)
            ...
            logs.error("Validation with Test Case No: 100283 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100283 failed")
            browser.implicitly_wait(5)
        finally:
