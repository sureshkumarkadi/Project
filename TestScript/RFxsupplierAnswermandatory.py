...
tf = 'test_RFxsupplierAnswerMandatory'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = RFx-11
class RFxsupplierAnswerMandatory(unittest.TestCase):
    def test_RFxsupplierAnswerMandatory(self):
        try:
            ...
            mandatorycheck1 = DataDriver()
            time.sleep(1)
            RFxmandatorycheck_path = mandatorycheck1.readfromXML(folder_path+'\Object\RFx.xml','eTender','mandatorycheck')
            RFxmandatorycheck = browser.find_element_by_xpath(RFxmandatorycheck_path)
            time.sleep(1)

            self.assertEqual(RFxmandatorycheck.text,'1. Have you ISO Certified? *')
