...
#filename = 'TestCase-100273-{0}.png'.format(ptime)
tf = 'test_Plantemplatestagdelete'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100273
class Plantemplatestagdelete(unittest.TestCase):
    def test_Plantemplatestagdelete(self):
        try:
            ...
            browser = tendertemplate.stagdelete(browser) #Stag delete
            time.sleep(2)
            ...
            if env == 'StageURL':
                ...
                self.assertEqual(templatestagescount,'6')
            ...
            logs.info("Test Case No : 100273 Passed Successfully")
        except Exception:
            logs.error("Validation with Test Case No: 100273 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100273 failed")
            browser.implicitly_wait(5)
        finally:
            time.sleep(1)
