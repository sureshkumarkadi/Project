...
#filename = 'TestCase-100316-{0}.png'.format(ptime)
tf = 'test_Tenderlistviewafterdeleteitem'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100316
class Tenderlistviewafterdeleteitem(unittest.TestCase):
    def test_Tenderlistviewafterdeleteitem(self):
        try:
            ...
            logs.info("Test Case No : 100316 Passed Successfully")
            time.sleep(1)
            browser = itemdetails.tenderproject(browser)
            time.sleep(1)
            browser = tendertemplate.TenderDeletion(browser)
            time.sleep(1)
        except Exception:
            logs.error("Validation with Test Case No: 100316 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100316 failed")
            browser.implicitly_wait(5)
        finally:
