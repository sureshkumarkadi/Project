...
tf = 'test_RFxQuestionstabclick'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = RFX-1b
class RFxQuestionstabclick(unittest.TestCase):
    def test_RFxQuestionstabclick(self):
        try:
            ...
            time.sleep(1)
            RFxaddquestion_path = RFxaddquestion1.readfromXML(folder_path+'\Object\RFx.xml','eTender','addquestion')
            ...
            addquestionserialnum = browser.find_element_by_xpath(addquestionserialnum_path)
            time.sleep(1)
            self.assertEqual(addquestionserialnum.text,'1.')

            RFxcheckboxes_path = RFxaddquestion1.readfromXML(folder_path+'\Object\RFx.xml','eTender','checkboxes')
            RFxcheckboxes = browser.find_elements_by_xpath(RFxcheckboxes_path)
            time.sleep(1)
            self.assertEqual(RFxcheckboxes[0].text,'Additional response required')
