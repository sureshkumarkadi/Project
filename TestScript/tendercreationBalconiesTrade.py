...
#filename = 'TestCase-100362-{0}.png'.format(ptime)
tf = 'test_TendercreationBalconiesTrade'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100362
class TendercreationBalconiesTrade(unittest.TestCase):
    def test_TendercreationBalconiesTrade(self):
        try:
            i = 1
            while i<2:
                ...
                self.assertEqual(tenderstatus[2].text,'Active')
                #self.assertEqual(tendertype,'TRADE')
                #self.assertEqual(tenderstatus[10].text,todaysdate)
                ...
                #self.assertEqual(tendername[4].text,'Balconies')

                time.sleep(3)
                i = i + 1
        except Exception:
                logs.error("Validation with Test Case No: 100362 failed")
                browser.save_screenshot(fullpath)
                traceback.print_exc(file=sys.stdout)
                self.fail("Test Case No: 100362")
                browser.implicitly_wait(5)
        finally:
