...
#filename = 'TestCase-100259-{0}.png'.format(ptime)
tf = 'test_Plantemplateactualresetplanned'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100259
class Plantemplateactualresetplanned(unittest.TestCase):
    def test_Plantemplateactualresetplanned(self):
        try:
            ...
            browser = tendertemplate.resetplanned(browser) #Select Reset Planned
            time.sleep(1)
            ...
            self.assertEqual(templateplanneddates[1].text,forecastdatestag1.strip())
            self.assertEqual(templateplanneddates[2].text,forecastdatestag2.strip())
            self.assertEqual(templateplanneddates[3].text,forecastdatestag3.strip())
            self.assertEqual(templateplanneddates[4].text,forecastdatestag4.strip())
            self.assertEqual(templateplanneddates[5].text,forecastdatestag5.strip())
            self.assertEqual(templateplanneddates[6].text,forecastdatestag6.strip())

            logs.info("Test Case No : 100259 Passed Successfully")
        except Exception:
            logs.error("Validation with Test Case No: 100259 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100259 failed")
            browser.implicitly_wait(5)
        finally:
            time.sleep(1)
