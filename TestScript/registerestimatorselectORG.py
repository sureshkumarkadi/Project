...
#filename = 'TestCase-100166-{0}.png'.format(ptime)
tf = 'test_registerestimatorselectorg'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100166
class RegisterestimatorselectORG(unittest.TestCase):
    def test_registerestimatorselectorg(self):
        try:
            ...
            time.sleep(1)
            browser = registration.selectestimatorRole(browser)
            time.sleep(1)
...
##            time.sleep(1)
            logs.info("Test Case No : 100166 Passed Successfully")
        except Exception:
            logs.error("Validation with Test Case No: 100166 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100166 failed")
            browser.implicitly_wait(5)
        finally:
