...
#filename = 'TestCase-100165-{0}.png'.format(ptime)
tf = 'test_registerestimatorneworg'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100165
class RegisterestimatornewORG(unittest.TestCase):
    def test_registerestimatorneworg(self):
        try:
            ...
            time.sleep(1)
            logs.info("Test Case No : 100165 Passed Successfully")
        except Exception:
            logs.error("Validation with Test Case No: 100165 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100165 failed")
            browser.implicitly_wait(5)
        finally:
