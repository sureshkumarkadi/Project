...
#filename = 'TestCase-100262-{0}.png'.format(ptime)
tf = 'test_Plantemplatenotesforallstages'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100262
class Plantemplatenotesforallstages(unittest.TestCase):
    def test_Plantemplatenotesforallstages(self):
        try:
            ...
            time.sleep(1)
            browser = tendertemplate.addingNotesfortenderplan(browser) # adding notes
            time.sleep(1)
            browser = tendertemplate.notesfortenderplan(browser) #Click on Notes
            time.sleep(1)
            ...
            logs.info("Test Case No : 100262 Passed Successfully")
            time.sleep(1)
            ...
            logs.error("Validation with Test Case No: 100262 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100262 failed")
            browser.implicitly_wait(5)
        finally:
