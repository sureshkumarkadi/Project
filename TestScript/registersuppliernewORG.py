...
#filename = 'TestCase-100163-{0}.png'.format(ptime)
tf = 'test_registersupplierneworg'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100163
class RegistersuppliernewORG(unittest.TestCase):
    def test_registersupplierneworg(self):
        try:
            ...
            time.sleep(1)
            logs.info("Test Case No : 100163 Passed Successfully")
        except Exception:
            logs.error("Validation with Test Case No: 100163 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100163 failed")
            browser.implicitly_wait(5)
        finally:
