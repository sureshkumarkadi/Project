#-------------------------------------------------------------------------------
# Name:         AT_TaskSchedulerSave.py
#
# Purpose:      To verify Save functionality in Task Scheduler
#
# Tast Case Id  100179
#
# Author:      mathew.jacob
...
#filename = 'TestCase-100179-{0}.png'.format(ptime)
tf = 'test_TaskSchedulerSave'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

class SaveTaskScheduler(unittest.TestCase):
    def test_TaskSchedulerSave(self):
        try:
            ...
            browser.find_element_by_xpath(orgLink.readfromXML(folder_path+'\Object\AdminEstimator.xml','eTender','SaveButton')).click()
            time.sleep(2)
            ...
            self.assertEquals(int(DueValueLatest),DueNewvalue)
            self.assertEquals(int(AcceptLatest),Acceptnewreminder)
        except Exception:
            logs.error("Validation with Test Case No: 100179 failed")
            browser.save_screenshot(fullpath)
            traceback.print_exc(file=sys.stdout)
            self.fail("Test Case No: 100179 failed")
