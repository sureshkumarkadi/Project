...
#filename = 'TestCase-Infrastructures-{0}.png'.format(ptime)
tf = 'test_updateprojectdetailsInfrastructures'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = Infrastructures
class UpdateprojectdetailsInfrastructures(unittest.TestCase):
    def test_updateprojectdetailsInfrastructures(self):
        try:
            ...
            while i<5:
                ...
                env = Infrastructures1.readfromXML(folder_path+'\Env\Setup.xml','eTender','env')

                if env == 'StageURL':

                    Infrastructures_path = Infrastructures1.readfromXML(folder_path+'\Object\PairwiserupdateprojectStaging.xml','eTender','Infrastructures') #Validate image id
                    Infrastructures = browser.find_element_by_xpath(Infrastructures_path)
                    ...
                    Infrastructures_path = Infrastructures1.readfromXML(folder_path+'\Object\Pairwiserupdateproject.xml','eTender','Infrastructures') #Validate image id
                    Infrastructures = browser.find_element_by_xpath(Infrastructures_path)
                    time.sleep(2)
                    if Infrastructures.is_displayed():
                        print("pass")
