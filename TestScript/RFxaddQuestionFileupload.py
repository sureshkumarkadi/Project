...
tf = 'test_RFxaddQuestionfileupload'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = RFX-4
class RFxaddQuestionfileupload(unittest.TestCase):
    def test_RFxaddQuestionfileupload(self):
        try:
            ...
            time.sleep(1)
            disabledpathforuploadfile = browser.find_elements_by_xpath(RFxupload.readfromXML(folder_path+'\Object\RFx.xml','eTender','disabled'))
            time.sleep(1)
