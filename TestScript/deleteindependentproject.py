...
#filename = 'TestCase-100193-{0}.png'.format(ptime)
tf = 'test_deleteindependentproject'
filename = 'Testcase-%s.png' %(tf)
path= setupValue().screenpath
fullpath = os.path.join(path,filename)

#Test case Number = 100193
class Deleteindependentproject(unittest.TestCase):
    def test_deleteindependentproject(self):
        try:
            i = 10
            while i<11:
                ...
                time.sleep(3)
                browser = Updatedetails.deleteproject(browser) # delete project
                time.sleep(2)
                ...
                self.assertIsNot(projectName.text,'ProjectchangeName1')
                time.sleep(1)

                i = i + 1
                logs.info("Test Case No : 100193 Passed Successfully")
        except Exception:
                logs.error("Validation with Test Case No: 100193 failed")
                browser.save_screenshot(fullpath)
                traceback.print_exc(file=sys.stdout)
                self.fail("Test Case No: 100193 failed")
                browser.implicitly_wait(5)
        finally:
